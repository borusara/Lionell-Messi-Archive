...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is part of The Primes\nI can't act against our own.",
        )
        return
    ...
    if int(user_id) in DEMONS:
        message.reply_text(
            "OOOH someone's trying to gban a Staff *grabs popcorn*",
        )
        return
    ...
    if EVENT_LOGS:
        ...
        send_to_list(
            bot,
            DRAGONS + DEMONS,
            f"Assasinated The User (User banned in <code>{gbanned_chats}</code> chats)",
            html=True,
        )
    ...
    if gban_time > 60:
        ...
        message.reply_text("Assasinated The User", parse_mode=ParseMode.HTML)
    else:
        message.reply_text("Assasinated The User", parse_mode=ParseMode.HTML)
...
GBAN_HANDLER = CommandHandler("assasinate", gban, run_async=True)
UNGBAN_HANDLER = CommandHandler("spare", ungban, run_async=True)
GBAN_LIST = CommandHandler("assasinations", gbanlist, run_async=True)
